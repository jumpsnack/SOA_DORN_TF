...
def initial_block(inputs, is_training=True, scope='initial_block'):
    ...
    # Max pool branch
    net_pool = slim.max_pool2d(inputs, [2, 2], stride=2, padding='SAME', scope=scope + '_max_pool')
...
def ENet(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with slim.arg_scope([initial_block, bottleneck], is_training=is_training), \
            ...
            # return net, slim.get_trainable_variables(scope=scope)
            ...
            with slim.arg_scope([bottleneck], regularizer_prob=0.1, decoder=True):
                ...
                if not do_upsampling:
                    output_depth *= 2

                # The decoder section, so start to upsample.
                net = bottleneck(net, output_depth=output_depth, filter_size=3, upsampling=do_upsampling,
                ...
                net = bottleneck(net, output_depth=output_depth, filter_size=3, scope=bottleneck_scope_name + '_1')
                net = bottleneck(net, output_depth=output_depth, filter_size=3, scope=bottleneck_scope_name + '_2')
            ...
            # logits = tf.image.resize_bilinear(logits, size=[inputs_shape[1], inputs_shape[2]])
            # probabilities = tf.nn.softmax(logits, name='logits_to_softmax')
        # net = tf.image.resize_bilinear(net, size=[49,65])
        return net, slim.get_trainable_variables(scope=scope)
